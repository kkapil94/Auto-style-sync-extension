...
function activate(context) {
  context.subscriptions.push(
    commands.registerCommand("vs-html-x-css.generate", generateCssFromHtml)
  );
  ...
  if (workspace.workspaceFolders.length !== 0) {
    ...
    const configCommand = commands.registerCommand(
      "htmlToCssConfig",
      async function () {
        // show configuration menu UI on command activation
        const configMenuInput = await window.showQuickPick(
          Object.keys(configInputMethods)
        );
        ...
        if (configMenuInput) {
          ...
          if (configInput) {
            window.showInformationMessage(
              "HTML to CSS autocompletion: Configuration changes are now active."
            );
            configInput = "";
          }
        }
...
function generateCssFromHtml() {
  if (window.activeTextEditor) {
    ...
    window.activeTextEditor.selections.forEach((element) => {
      let selectedText = window.activeTextEditor.document.getText(element);
    ...
    if (allSelectedText) {
      ...
      if (allClass.isNotEmpty()) {
        let clipboard = "";
        let config = workspace.getConfiguration("vscode-html-x-css");
        ...
        if (!config.get("disableNotification")) {
          window.showInformationMessage("CSS boilerplate copied to clipboard.");
        }
      }
    }
  }
...
function getPathList(workspaceFolder) {
  ...
  const fileTypesStr = getFromFileTypesConfig.reduce((str, fileType, ind) => {
    ...
    return ind === getFromFileTypesConfig.length - 1 &&
      getFromFileTypesConfig.length > 0
      ? "{" + str + "," + fileType + "}"
      : (str += "," + fileType);
  });
  ...
  if (workspaceFolder) {
    if (includeConfig) {
      ...
      includeStr = includeFoldersConfig.reduce((str, folder, ind) => {
        return ind === includeFoldersConfig.length - 1 &&
          includeFoldersConfig.length > 0
          ? "{" + str + "," + folder + "}"
          : (str += "," + folder);
      });
      include = new RelativePattern(
        workspaceFolder,
        `**/${includeStr}/*.${fileTypesStr}`
      );
    } else {
  ...
  if (excludeConfig) {
    ...
    excludeStr = excludeFoldersConfig.reduce((str, folder, ind) => {
      return ind === excludeFoldersConfig.length - 1 &&
        excludeFoldersConfig.length > 0
        ? "{" + str + "," + folder + "}"
        : (str += "," + folder);
    });
    exclude = `**/${excludeStr}/**`;
...
function parseData(fileObj) {
  ...
  let regex =
    /(class|id|rel=["'].*(stylesheet).*["'].+href)=["']([^"']+)["']/gi;
  let match;
  let selector;
...
function registerItemProvider(languageFilter) {
  ...
  const providerFunction = {
    provideCompletionItems: (document, position, token, context) => {
      ...
      if (
        (triggerCharsBool && context.triggerKind != 1) ||
...
function registerHoverProvider(languageFilter) {
  const hoverProvider = languages.registerHoverProvider(languageFilter, {
    provideHover: (document, position, token) => {
      ...
      const selectorLeft = line
        .slice(null, position.character)
        .match(/[\#\.][\w_-]*$/);
...
function setFSWatcher(includePattern) {
  ...
  globWatcher.onDidChange((uri) => {
    if (files.hasOwnProperty(uri.fsPath) && files[uri.fsPath].isProcessing)
      return;
    getFiles(uri.fsPath);
  });
...
const configInputMethods = {
  ...
  "Set autocompletion workspace scope": {
    ...
    set: async function () {
      configInput = await window.showQuickPick([
        "multi-root",
        "workspace",
        "linked-files",
      ]);
  ...
  "Set file types to be searched for classes/ids": {
    ...
    set: async function () {
      configInput = await window.showInputBox({
        prompt:
          "Set file types to be searched for classes/ids. E.g.: html, php",
        placeHolder: "html",
      });
  ...
  "Set list of include folders": {
    ...
    set: async function () {
      configInput = await window.showInputBox({
        prompt:
          "Sets folders to be searched for file types. E.g.: app, folderName, folderName2",
      });
  ...
  "Set list of exclude folders": {
    ...
    set: async function () {
      configInput = await window.showInputBox({
        prompt:
          "Sets folders to be excluded from searching. E.g.: app, folderName, folderName2",
      });
  ...
  "Set include glob pattern": {
    ...
    set: async function () {
      configInput = await window.showInputBox({
        prompt:
          "Set include glob pattern. E.g.: **/{folderName1,folderName2,...}/*.{fileType1,fileType2,...}",
      });
  ...
  "Set exclude glob pattern": {
    ...
    set: async function () {
      configInput = await window.showInputBox({
        prompt:
          "Set exclude glob pattern. E.g.: **/{folderName1,folderName2,...}/**",
      });
  ...
  "Restore configurations to default": function () {
    ...
    registerProviders();
    window.showInformationMessage(
      "HTML to CSS autocompletion: All files have been parsed."
    );
...
async function askToDefault(configObj) {
  const checkInput = await window.showQuickPick([
    "Restore to default",
    "Cancel",
  ]);
